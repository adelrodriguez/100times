...
function FieldLabel(props: React.ComponentProps<typeof LabelPrimitive.Root>) {
  ...
  return <Label {...props} className={cn(hasError && "text-destructive", props.className)} htmlFor={field.name} />
}
FieldLabel.displayName = "FieldLabel"

function FieldDescription(props: React.ComponentProps<"div">) {
  return <div {...props} className={cn("text-[0.8rem] text-muted-foreground", props.className)} />
}
...
function FieldMessage(props: React.ComponentProps<"div">) {
  ...
  return (
    <div {...props} className={cn("font-medium text-[0.8rem] text-destructive", props.className)}>
      {body}
    </div>
  )
}
...
function FormServerError({ title = "Error", ...props }: React.ComponentProps<"div"> & { title?: string }) {
  const form = useFormContext()
